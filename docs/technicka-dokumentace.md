...
Web (FastAPI + Jinja2 + SSE)
  └─ spustí LangGraph pipeline (6 agentů)
...
- konkrétní agenti: `law_identifier.py`, `head_classifier.py`, `paragraph_selector.py`, `qualifier.py`, `special_law_checker.py`, `reviewer.py`
...
│
│── special_law_kvalifikace: list[dict] ◄── Agent 4
│── special_law_notes: list[str]   ◄── Agent 4
│
│── final_kvalifikace: list[dict]  ◄── Agent 5
│── review_notes: list[str]        ◄── Agent 5
│
...
#### Agent 4: Special Law Checker

- **Účel**: Identifikace porušení speciálních zákonů (zákon o zbraních, o návykových látkách, dopravní zákon aj.) nad rámec hlavní TZ kvalifikace.
- **MCP volání**: `search_laws(query=popis, top_k=10)` → filtr na `typ=="specialni"` → `search_chapters` + `search_paragraphs` + `get_paragraph_text` pro každý nalezený speciální zákon.
- **LLM analyzuje**: Které paragrafy speciálních zákonů jsou skutkem porušeny. Okolnosti vylučující protiprávnost (nutná obrana) se vztahují jen na TZ kvalifikaci — porušení speciálního zákona je samostatný skutek.
- **Filtr**: confidence > 0.3.
- **Výstup**:
  - `special_law_kvalifikace = [{ paragraf, nazev, zakon, confidence, duvod, chybejici_znaky }]`
  - `special_law_notes = ["Nutná obrana nevylučuje přestupek neoprávněného držení zbraně", ...]`

#### Agent 5: Reviewer

- **Účel**: Křížová kontrola konzistence, úplnosti a právní správnosti. Kontroluje i kvalifikace ze speciálních zákonů (Agent 4).
- **MCP volání**: žádné — pracuje čistě s LLM nad existujícím stavem.
- **LLM kontroluje 6 oblastí**:
  ...
  5. **Speciální zákony** — konzistence s kvalifikacemi ze speciálních zákonů (Agent 4), duplikáty, protimluvy.
  6. **Review notes** — dokumentace každé úpravy s odůvodněním.
- **Výstup**:
...
| 4: Special Law Checker | `search_laws`, `search_chapters`, `search_paragraphs`, `get_paragraph_text` | 1 + N×3 (N = nalezených speciálních zákonů) |
| 5: Reviewer | — | žádné |
